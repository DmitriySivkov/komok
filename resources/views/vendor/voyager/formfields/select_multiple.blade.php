...
<select class="form-control select2" name="{{ $row->field }}[]" multiple>
    @if(isset($options->customAllFromRelation))
        <?php $relationshipListMethod = \Illuminate\Support\Str::camel($row->field) . 'List'; ?>
        <?php
            if (request()->route()->getName() != 'voyager.' . $dataType->slug . '.create')
                $selected_values = isset($dataTypeContent) ? $dataTypeContent->{$row->field} : [];
            else
                $selected_values = [];
        ?>
        <?php
